...
This bus is separate from :mod:`jenny.bus.queue`: message bus events are
...
class SessionTurnStarted:
    ...
    ``content``/``media`` trasportano l'input utente del turno così gli adapter
    di vista (es. WebUI) possono rispecchiarlo senza conoscere il canale d'origine.
    """
...
class RuntimeEventPublisher:
    ...
    def runtime_model_changed(
        ...
        self.bus.publish_nowait(
            RuntimeModelChanged(model=model, model_preset=model_preset, provider=provider)
        )
