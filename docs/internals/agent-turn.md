...
4. **Recurring work** — where a repeating request belongs: a line in `HEARTBEAT.md`, a `cron` job in `reminder` mode, or one in `monitor` mode. Rendered only when the turn's registry actually contains the `cron` tool, so the substituted registries (Dream, Atlas) are not told to schedule with something they do not have.
