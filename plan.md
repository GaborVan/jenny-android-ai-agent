...
| [`roadmap/progetti-passi.md`](roadmap/progetti-passi.md) — la lista dei passi; [`roadmap/project-sessions.md`](roadmap/project-sessions.md) — l'archivio del perché | **In progress — through step 1 of `roadmap/progetti-passi.md`, verified on the device 2026-08-21** (open a project and it is its own conversation: own session, own thread, own folder, identity intact, writes confined). Earlier the same day, Phase 0 shipped (the Dream read filter that keeps the personal diary personal, and the collapse of the user's two session keys, which had been quietly running WebUI-created reminders in a session of their own). Fronted by a tickable *The workflow* checklist: **8 of 27 boxes done** — including **the session boundary (2026-08-21)**: `session_kind()` makes the classification ternary in one place, and a project turn neither writes nor reads `memory/history.jsonl`, built before anything can produce a `project:` key so that binding the folder turns on something already safe — the last two were pulled forward on 2026-08-21 because they were wrong against the wikis already on the device: `scaffold.py`'s `_write()` was an unconditional clobber, and the scope chip listed a `projects/` directory that does not exist while its *New* made a bare folder instead of a wiki (now `project.create` over the WS RPC, which scaffolds and takes the user's scope line). Shipped and verified on the device in `b49309c`, which also measured that **three of the seven wikis have no `CLAUDE.md` at all** — the top-up has work waiting. **A project is a wiki**: `wikis/<name>/` already is the entity, so no `projects/` is created. The UI half — scope chip + picker — is **built and on the device**. A 2026-08-21 pass against the owner's own framing (*Claude Code, writing a wiki instead of code*) settled three clashes — a project's agent **writes**, the boundary is on **writes only** (which dissolved the old blocker, item 14), and `CLAUDE.md` gives way to `AGENTS.md` — and left three as open questions |
