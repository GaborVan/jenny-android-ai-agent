...
| [`roadmap/project-sessions.md`](roadmap/project-sessions.md) | **In progress — Phase 0 shipped 2026-08-21** (the Dream read filter that keeps the personal diary personal, and the collapse of the user's two session keys, which had been quietly running WebUI-created reminders in a session of their own). Fronted by a tickable *The workflow* checklist: **4 of 27 boxes done**. **A project is a wiki**: `wikis/<name>/` already is the entity, so no `projects/` is created. The UI half — scope chip + picker — is **built and on the device**. A 2026-08-21 pass against the owner's own framing (*Claude Code, writing a wiki instead of code*) settled three clashes — a project's agent **writes**, the boundary is on **writes only** (which dissolved the old blocker, item 14), and `CLAUDE.md` gives way to `AGENTS.md` — and left three as open questions |
