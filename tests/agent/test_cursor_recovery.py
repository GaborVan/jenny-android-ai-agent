...
class TestNextCursorRecovery:
    ...
    def test_kill_between_append_and_cursor_write(self, store):
        """Crash after the entry fsync but before the ``.cursor`` write.

        ``append_history`` persists the entry (cursor N) first, then rewrites
        ``.cursor``.  A kill in between leaves ``.cursor`` at N-1.  Trusting
        ``.cursor`` alone would re-allocate N — a duplicate.  ``_next_cursor``
        must instead honour the last persisted entry and return N+1.
        """
        n = 7
        store.history_file.write_text(
            f'{{"cursor": {n}, "timestamp": "2026-04-01 10:00", "content": "last persisted"}}\n',
            encoding="utf-8",
        )
        # Simulate the stale cursor file left by the crash.
        store._cursor_file.write_text(str(n - 1), encoding="utf-8")
        cursor = store.append_history("after crash")
        assert cursor == n + 1  # 8, never the duplicate 7

    def test_cursor_file_ahead_of_last_entry(self, store):
        """Inverse case: ``.cursor`` higher than the last entry (e.g. history
        externally truncated).  ``max`` must keep monotonicity: never reuse a
        cursor, so the next value follows the ``.cursor`` file.
        """
        n = 20
        store.history_file.write_text(
            f'{{"cursor": {n}, "timestamp": "2026-04-01 10:00", "content": "trailing"}}\n',
            encoding="utf-8",
        )
        store._cursor_file.write_text(str(n + 5), encoding="utf-8")
        cursor = store.append_history("after truncation")
        assert cursor == n + 6  # 26, follows the higher .cursor
