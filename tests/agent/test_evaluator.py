...
async def test_should_notify_true() -> None:
    ...
    result = await evaluate_response(
        "Task completed with results", "check emails", provider, "m", default_notify=False
    )
...
async def test_should_notify_false() -> None:
    ...
    result = await evaluate_response(
        "All clear, no updates", "check status", provider, "m", default_notify=False
    )
...
async def test_fallback_on_error() -> None:
    ...
    result = await evaluate_response(
        "some response", "some task", provider, "m", default_notify=True
    )
    assert result is True
