...
class TestNewCommandArchival:
    ...
    @pytest.mark.asyncio
    async def test_schedule_background_logs_exception_instead_of_swallowing_it(
        self, tmp_path: Path, caplog
    ) -> None:
        """A background task that raises must produce a log record, not vanish silently."""
        import logging
        ...
        async def _raising_background_job() -> None:
            raise RuntimeError("boom: consolidation blew up")

        handler_id = loguru_logger.add(caplog.handler, format="{message}", level="ERROR")
        try:
            with caplog.at_level(logging.ERROR):
                loop._schedule_background(_raising_background_job())
                await loop.close_background_tasks()
        finally:
        ...
        assert len(error_records) == 1, (
            f"Expected exactly one error record for the failed background task, "
            f"got {len(error_records)}: {[r.getMessage() for r in error_records]}"
        )
        assert "_raising_background_job" in error_records[0].getMessage()
        assert loop._background_tasks == []

    @pytest.mark.asyncio
    async def test_schedule_background_stays_quiet_when_task_succeeds(
        self, tmp_path: Path, caplog
    ) -> None:
        """A background task that completes normally must not emit any error log."""
        import logging
        ...
        async def _ok_background_job() -> None:
            completed.set()

        handler_id = loguru_logger.add(caplog.handler, format="{message}", level="ERROR")
        try:
            with caplog.at_level(logging.ERROR):
                loop._schedule_background(_ok_background_job())
                await loop.close_background_tasks()
        finally:
            loguru_logger.remove(handler_id)
