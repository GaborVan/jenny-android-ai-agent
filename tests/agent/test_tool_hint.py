...
class TestToolHintKnownTools:
    ...
    def test_edit_file_shows_path(self):
        result = _hint([_tc("edit_file", {"path": "src/main.py", "old_text": "x", "new_text": "y"})])
        assert result == "edit src/main.py"

    def test_apply_patch_degrades_without_top_level_path(self):
        result = _hint([_tc("apply_patch", {"edits": [{"path": "a.py", "action": "replace"}]})])
    ...
    def test_exec_shows_command(self):
        result = _hint([_tc("python_exec", {"code": "npm install typescript"})])
        assert result == "py npm install typescript"
    ...
    def test_exec_abbreviates_paths_in_command(self):
        ...
        cmd = "cd D:\\Documents\\GitHub\\my-agent-project\\.worktree\\tomain\\checkout && git diff origin/main...pr-2706 --name-only 2>&1"
        result = _hint([_tc("python_exec", {"code": cmd})])
...
class TestToolHintMaxLength:
    ...
    def test_python_exec_default_truncates_at_40(self):
        cmd = "cd /very/long/path/to/some/project && npm run build && npm test"
        result = _hint([_tc("python_exec", {"code": cmd})], max_length=40)
        assert len(result) <= 50  # "$ " prefix + 40 + ellipsis
        assert "\u2026" in result

    def test_python_exec_larger_max_length_shows_more(self):
        cmd = "cd /very/long/path/to/some/project && npm run build && npm test"
        short = _hint([_tc("python_exec", {"code": cmd})], max_length=40)
        long = _hint([_tc("python_exec", {"code": cmd})], max_length=120)
        assert len(long) > len(short)
        assert "npm test" in long

    def test_python_exec_max_length_120_shows_full_command(self):
        cmd = "cd /home/user/project && npm install && npm run build"
        result = _hint([_tc("python_exec", {"code": cmd})], max_length=120)
    ...
    def test_list_dir_path_respects_max_length(self):
        ...
        long_path = "/home/user/.local/share/uv/tools/my-agent-project/"
