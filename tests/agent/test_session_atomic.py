...
class TestAtomicSave:
    ...
    def test_tmp_file_cleaned_up_on_write_failure(self, tmp_path: Path):
        # atomic_write usa nomi tmp univoci (uuid): in caso di errore deve
        # ripulire il proprio tmp, senza lasciarne in giro.
        mgr = SessionManager(tmp_path)
        ...
        with unittest.mock.patch("os.replace") as mock_replace:
            mock_replace.side_effect = OSError("simulated disk full")
            try:
                mgr.save(session)
            except OSError:
                pass
