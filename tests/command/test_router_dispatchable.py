...
class TestIsDispatchableCommand:
    ...
    def test_status_is_priority_only(self, router: CommandRouter) -> None:
        # /status è servito solo dal tier priority: non deve essere anche
        # registrato come exact (la registrazione exact sarebbe irraggiungibile).
        assert router.is_priority("/status")
        assert not router.is_dispatchable_command("/status")
